...
function calcScore(data) {
  ...
  let score = 50;
  if (inc > 0) { score += Math.min(25, (sav / inc) * 100); score -= Math.min(20, (exp / inc) * 40); }
  score += Math.min(15, noSpend * 2); score -= Math.min(15, regrets * 3);
  if (sav >= data.monthlyGoal) score += 10;
...
.bnav-icon{font-size:14px;transition:transform .2s;}
...
@media(min-width:768px){
  ...
  .sb-icon{font-size:17px;width:22px;text-align:center;transition:transform .2s;}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try { const c = await dbGet(); if (c) loaded = c; } catch {}
      if (loaded) {
        // Deep-merge with defaults so new fields added in future versions don't break old saves
        const merged = { ...mkDefault(), ...loaded,
          settings: { ...mkDefault().settings, ...(loaded.settings||{}) }
        };
        setData(merged);
      }
      setLoading(false);
  ...
  const props = { data, save, readOnly, setReadOnly, showToast };

  const currentTab = TABS.find(t => t.id === tab);
  const statusLine = syncing
    ? <><span className="dot-pulse" style={{ marginRight:5 }}/>Syncing...</>
    : "● Online";
    ...
    <>
      ...
      <div className="rf-root">
        ...
        <div className="desktop-wrap">
          <div className="rf-header">
            <div>
              <div className="logo" style={{ fontSize:15 }}>{currentTab?.icon} {currentTab?.label}</div>
              <div className="logo-sub">{statusLine}</div>
            </div>
            {headerRight}
          </div>
...
function Home({ data, save, showToast }) {
  ...
  const balance=totalInc-totalExp-mSaved-data.lockedMoney;
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontFamily:"'DM Mono',monospace",fontSize:"clamp(26px,4vw,42px)",fontWeight:500,color:"#fff",lineHeight:1.1 }}>
              Rs. <AnimCounter target={Math.max(0,Math.round(balance))}/>
            </div>
...
/* ── SETTINGS ── */
function Settings({ data, save, readOnly, setReadOnly, showToast }) {
  ...
  const saveSettings=()=>{
    if(readOnly){showToast("Unlock first","err");return;}
    // Sanitize new key — no HTML, min 4 chars if provided
    const cleanKey=(newKey||"").replace(/<[^>]*>/g,"").trim();
    if(cleanKey&&cleanKey.length<4){showToast("Write key must be ≥ 4 characters","err");return;}
    save({
      ...data,
      ...
      settings:{
        ...data.settings,
        writeKey:cleanKey||data.settings?.writeKey||WRITE_KEY,
      },
    });
  ...
  const exportCSV=()=>{
    ...
    const rows=[["Date","Type","Category","Amount","Note","Worth","Regret"]];
    data.transactions.forEach(t=>rows.push([t.date,t.type,t.category,t.amount,esc(t.note||""),t.worth||"",t.regret?"yes":"no"]));
  ...
  const factoryReset=async()=>{
    ...
    // Wipe Upstash Redis
    await dbDelete();
    // Reset in-memory state — preserve settings so db stays configured
    save({...fresh, settings: data.settings});
        ...
        <div>
          {!readOnly&&(
            <div className="card">
              ...
              {[{l:"Monthly Income (Rs.)",v:income,s:setIncome},{l:"Monthly Saving Goal (Rs.)",v:goal,s:setGoal},{l:"Locked Money (Rs.)",v:locked,s:setLocked}].map(f=>(
              ...
              <div style={{ marginBottom:12 }}>
                <div style={{ fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(255,255,255,.3)",marginBottom:6,letterSpacing:".08em" }}>CHANGE WRITE KEY (blank = keep)</div>
                <input type="password" className="inp" placeholder="New write key..." value={newKey} onChange={e=>setNewKey(e.target.value)}/>
              </div>
              <div style={{ marginBottom:10,padding:"10px 14px",background:"rgba(16,185,129,.06)",border:"1px solid rgba(16,185,129,.15)",borderRadius:12 }}>
                <div style={{ fontFamily:"'DM Mono',monospace",fontSize:10,color:"#10b981",marginBottom:4,letterSpacing:".08em" }}>☁ CLOUD SYNC ACTIVE</div>
                <div style={{ fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(255,255,255,.3)",lineHeight:1.7 }}>Upstash Redis is configured server-side via Vercel environment variables. No credentials needed here.</div>
              </div>
