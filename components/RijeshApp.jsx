...
export default function App() {
  ...
  // Track whether a reset just happened so we don't let cloud overwrite fresh zeros
  ...
  useEffect(() => {
    (async () => {
      let loaded = null;
      setLoading(false);
      // Load from cloud only — localStorage removed, cloud is single source of truth
      if (justResetRef.current) return;
      try {
        const c = await dbGet();
        // Only apply cloud data if it has actual content (not empty/null)
        if (c && typeof c === "object" && !justResetRef.current) {
  ...
  // Poll cloud every 30s — skip if editing or just reset
  const readOnlyRef = useRef(true);
  useEffect(() => { readOnlyRef.current = readOnly; }, [readOnly]);
  useEffect(() => {
    const interval = setInterval(async () => {
      if (!readOnlyRef.current) return; // skip if user is actively editing
      if (justResetRef.current) return; // skip if factory reset just happened
      try {
        const c = await dbGet();
  ...
  const props = { data, save, readOnly, setReadOnly, showToast, syncing, lastSynced, forceSync, justResetRef };
...
/* ── SETTINGS ── */
function Settings({ data, save, readOnly, setReadOnly, showToast, syncing, lastSynced, forceSync, justResetRef }) {
  ...
  const factoryReset=async()=>{
    ...
    // ── FACTORY RESET ──────────────────────────────────────────
    // 1. Block ALL cloud polls immediately so nothing can overwrite us
    if (justResetRef) justResetRef.current = true;

    // 2. Build completely clean zeros state
    const resetData = mkDefault();

    // 3. No localStorage to clear (removed — cloud only)

    // 4. Update UI to zeros right away — user sees it instantly
    setData(resetData);
    setReadOnly(true);

    // 5. Delete cloud DB key
    const deleted = await dbDelete();

    // 6. Write zeros to cloud DB — retry up to 3 times to make sure it sticks
    let cloudOk = false;
    for (let attempt = 0; attempt < 3; attempt++) {
      cloudOk = await dbSet(resetData);
      if (cloudOk) break;
      await new Promise(r => setTimeout(r, 500)); // wait 500ms before retry
    }

    // 7. Verify — read back from cloud to confirm it's actually zeros
    let verified = false;
    try {
      const check = await dbGet();
      // Verify cloud now has zeros (walletBalance should be 0, transactions empty)
      if (check && check.walletBalance === 0 && Array.isArray(check.transactions) && check.transactions.length === 0) {
        verified = true;
      } else if (!check) {
        // null means key was deleted — that's also fine, write zeros again
        await dbSet(resetData);
        verified = true;
      }
    } catch {}

    // 8. Cloud is the only source of truth — no localStorage

    if (verified || cloudOk) {
      showToast("Factory reset complete — cloud + local both zeroed ✓");
    } else {
      showToast("Reset done locally — cloud may need manual sync", "err");
    }
  };
  const importJSON=(e)=>{
    if(readOnly){showToast("Unlock first","err");return;}
