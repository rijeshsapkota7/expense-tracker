...
function Home({ data, save, showToast }) {
  ...
  // Real balance = wallet starting balance + this month income - expenses - savings - locked
  const walletBase = data.walletBalance || 0;
  const balance = Math.max(0, walletBase + mT.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0) - totalExp - mSaved - data.lockedMoney);
    ...
    <div>
      ...
      <div style={{ display:"flex",gap:10,marginBottom:14,flexWrap:"wrap" }}>
        ...
        <div style={{ flex:1,minWidth:140,background:"rgba(99,102,241,.07)",border:"1px solid rgba(99,102,241,.18)",borderRadius:16,padding:"14px 18px",display:"flex",flexDirection:"column",gap:4 }}>
          ...
          <div style={{ fontFamily:"'DM Mono',monospace",fontSize:22,fontWeight:600,color:"#a78bfa" }}>{fmt(balance)}</div>
...
function Settings({ data, save, readOnly, setReadOnly, showToast, syncing, lastSynced, forceSync }) {
  ...
  const exportCSV=()=>{
    ...
    const rows=[["ID","Date","Type","Category","Amount","Note","Worth","Regret","CreatedAt"]];
    data.transactions.forEach(t=>rows.push([t.id,t.date,t.type,t.category,t.amount,esc(t.note||""),t.worth||"",t.regret?"yes":"no",t.createdAt||""]));
  ...
  const factoryReset=async()=>{
    ...
    const resetData = {...fresh, settings: data.settings};
    setData(resetData);
